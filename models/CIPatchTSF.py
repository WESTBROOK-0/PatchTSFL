...
from layers.CIPatchTSF_backbone import CIPatchTSF_backbone
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.model.append(CIPatchTSF_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride,
                                           moving_avg=moving_avg, e_layers=e_layers, d_model=d_model,
                                           ...
                                           pretrain_head=pretrain_head, head_type=head_type, individual=self.individual,
                                           revin=revin, affine=affine,
                                           subtract_last=subtract_last, verbose=verbose, **kwargs))
        else:
            self.model = CIPatchTSF_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride,
                                           moving_avg=moving_avg, e_layers=e_layers, d_model=d_model,
                                           n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout,
    ...
    def forward(self, x, attn_mask=None):  # x: [Batch, Input length, Channel]
        # x: [Batch, Input length, Channel]
        if self.individual:
            x_out = []
            x = x.permute(0, 2, 1)     #(128,21,96)
            #output = torch.zeros([x.size(0), x.size(1), self.pred_len], dtype=x.dtype).to(x.device)
            for i in range(self.channels):
                z = self.model[i](x[:, i, :], attn_mask=attn_mask)
                x_out.append(z)
            x = torch.cat(x_out,dim=1)
            x = x.permute(0, 2, 1)
        else:
            x = self.model(x.permute(0, 2, 1), attn_mask=attn_mask).permute(0, 2, 1)
        ...
        #     x = self.model(x, attn_mask=attn_mask)       #(128,21,96)
        #     x = x.permute(0, 2, 1)  # x: [Batch, Input length, Channel]
        # return x