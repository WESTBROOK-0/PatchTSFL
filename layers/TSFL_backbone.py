...
class TSFL_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        # Backbone
        self.backbone = TSFLiEncoder(c_in, context_window=context_window, moving_avg=moving_avg,
...
class TSFLiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, context_window, moving_avg=25,
                 e_layers=3, d_model=128, n_heads=16, d_k=None, d_v=None,
        ...
        encoder_self_att = FourierBlock(n_heads=n_heads, in_channels=d_model, out_channels=d_model, seq_len=q_len, modes=64, mode_select_method='random')
        #self.encoder = FourierBlock(in_channels=d_model, out_channels=d_model, seq_len=q_len, modes=0, mode_select_method='random')
    ...
    def forward(self, x,  attn_mask=None) -> Tensor:  # x: [bs x nvars x patch_len x patch_num] # x: [bs x nvars x seq_len]

        n_vars = x.shape[1]
        # Input encoding
        x = x.unsqueeze(-1)                       #(128,21,96,1)
        x = x.unsqueeze(-1)                            #(128,21,96,1,1)
        x = x.expand(-1,-1,-1,-1,self.d_model)        #(128,21,96,1,512)
        x = x.squeeze(-2)                                #(128,21,96,512)
        ...
        # Encoder
        z,_ = self.encoder(u, attn_mask=attn_mask)  # z: [bs * nvars x patch_num x d_model]
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer in self.attn_layers:
                x, attn = attn_layer(x, attn_mask=attn_mask)      #(2688,12,512)
                attns.append(attn)
...
class FourierBlock(nn.Module):
    def __init__(self, n_heads, in_channels, out_channels, seq_len, modes=0, mode_select_method='random'):
