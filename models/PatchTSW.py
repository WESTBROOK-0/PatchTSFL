...
from layers.PatchTSW_backbone import PatchTSW_backbone
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        if self.decomposition:
            ...
            self.model_trend = PatchTSW_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
            ...
            self.model_res = PatchTSW_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                               patch_len=patch_len, stride=stride,
                                               moving_avg=moving_avg, e_layers=e_layers, d_model=d_model,
                                               n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                               attn_dropout=attn_dropout,
            ...
            self.model = PatchTSW_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride,
                                           moving_avg=moving_avg, e_layers=e_layers, d_model=d_model,
                                           n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout,
