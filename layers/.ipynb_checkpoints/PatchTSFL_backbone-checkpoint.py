...
class TSFLiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, context_window, patch_num, patch_len, moving_avg=25,
        ...
        encoder_self_att = FourierBlock(in_channels=d_model, out_channels=d_model, seq_len=q_len, modes=64, mode_select_method='random')
    ...
    def forward(self, x,  attn_mask=None) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        # Encoder
        z,_ = self.encoder(u, attn_mask=attn_mask)  # z: [bs x patch_num x d_model x nvars]
...
class FourierBlock(nn.Module):
    def __init__(self, in_channels, out_channels, seq_len, modes=0, mode_select_method='random'):
        ...
        """
        # get M modes on frequency domain
    ...
    def forward(self, q, k, v, mask):
        ...
        B, L, H, E = q.shape
        x = q.permute(0, 2, 3, 1)
        # Compute Fourier coefficients Q
        x_ft = torch.fft.rfft(x, dim=-1)
        # Perform Fourier neural operations
        out_ft = torch.zeros(B, H, E, L // 2 + 1, device=x.device, dtype=torch.cfloat)
        # padding  sampling Q*R
